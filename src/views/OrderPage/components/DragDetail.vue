...
import { Status } from "@/enums/medicine";
let route = useRoute();
let { detail } = useDragInfo(route.params.orderId as string);
</script>
...
<template>
  <div class="order-detail-page" v-if="detail">
    ...
    <div class="order-detail">
      <van-cell-group>
        <van-cell title="药品金额" :value="`￥${detail.payment}`" />
        <van-cell title="运费" :value="`￥${detail.expressFee}`" />
        ...
        <van-cell
          title="支付方式"
          :value="detail.paymentMethod === 0 ? '微信' : '支付宝'"
        />
      </van-cell-group>
    </div>
    <!-- 已取消 -->
    <van-action-bar v-if="detail.status === Status.OrderCancel">
      <van-action-bar-icon icon="delete-o" text="删除" />
      <van-action-bar-button type="primary" text="沟通记录" />
    </van-action-bar>
    <!-- 待收货 -->
    <van-action-bar v-if="detail.status === Status.GetGoods">
      <van-action-bar-button type="primary" text="确认收货" />
    </van-action-bar>
    <!-- 待发货 -->
    <van-action-bar v-if="detail.status === Status.WaitingSendGoods">
      <van-action-bar-button type="primary" text="提醒发货" />
    </van-action-bar>
    <!-- 待支付 -->
    <van-action-bar v-if="detail.status === Status.WaitingPay">
      <p class="price">需要支付：<span>￥60</span></p>
      <van-action-bar-button color="#bbb" text="取消问诊" />
      <van-action-bar-button type="primary" text="继续支付" />
    </van-action-bar>
    <!-- 已完成 -->
    <van-action-bar v-if="detail.status === Status.OrderSuccess">
      <van-action-bar-icon icon="delete-o" text="删除" />
